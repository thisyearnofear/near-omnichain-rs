...
export class ModernUIManager extends EventEmitter {
    ...
    validateForm() {
        ...
        const nearConnected = document.getElementById('near-wallet')?.classList.contains('connected');
        const baseConnected = document.getElementById('base-wallet')?.classList.contains('connected');

        const isValid = 
            nearConnected &&
            baseConnected &&
            amount && parseFloat(amount) > 0 &&
            recipient && this.isValidAddress(recipient);
    ...
    formatAccount(account) {
        ...
        return account.length > 20 ? 
            `${account.slice(0, 8)}...${account.slice(-8)}` : account;
    }
